...
```python
with Daq.open_task(spec) as session, record(session, chunk_size=1000) as (stream, summary):
    for block in stream:
        process(block)
...
with Daq.open_task(spec) as session, record_polled(session, rate_hz=2.0) as (stream, _summary):
...
- Run multiple sync sessions on the same portal — each `Daq.open_task`
